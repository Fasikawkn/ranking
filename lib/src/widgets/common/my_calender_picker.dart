...
///    [MyCalendarDatePicker] and provides an optional compact view where the
...
class _MyCalendarDatePickerState extends State<MyCalendarDatePicker> {
  ...
  @override
  void didUpdateWidget(MyCalendarDatePicker oldWidget) {
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    final Color controlColor = whiteColor;
    // var formate =  DateFormat.MONTH_DAY('')
    var date = intl.DateFormat('MMMM dd');
    var dateString = date.format(_currentMonth);
    return Semantics(
      child: Column(
        children: <Widget>[
          Container(
            ...
            child: Row(
              ...
              children: <Widget>[
                ...
                Text(dateString, style: kAppbarTextStyle),
                IconButton(
...
class _DayPickerState extends State<_DayPicker> {
  ...
  List<Widget> _dayHeaders(
    ...
    for (int i = localizations.firstDayOfWeekIndex; true; i = (i + 1) % 7) {
      ...
      result.add(ExcludeSemantics(
        child: Center(child: Text(weekday, style: kAppbarTextStyle)),
      ));
  ...
  Widget build(BuildContext context) {
    ...
    const Color enabledDayColor = whiteColor;
    final Color disabledDayColor = whiteColor.withOpacity(0.32);
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        BoxDecoration? decoration = BoxDecoration(
            border: Border.all(color: enabledDayColor, width: 0.2),
        ...
        if (isSelectedDay) {
          ...
          decoration = BoxDecoration(
              color: selectedDayBackground, shape: BoxShape.rectangle
              // shape: BoxShape.circle,
              );
        } else if (isDisabled) {
          dayColor = disabledDayColor;
          decoration = BoxDecoration(
              border: Border.all(color: disabledDayColor, width: 0.1),
              shape: BoxShape.rectangle);
        } else if (isToday) {
        ...
        Widget dayWidget = Container(
          ...
          child: Center(
            child: Text(localizations.formatDecimal(day),
                style: kAppbarTextStyle.copyWith(color: dayColor)),
          ),
        );
...
/// [MyCalendarDatePicker], or [showDatePicker] which create full date pickers.
///
/// See also:
///
///  * [MyCalendarDatePicker], which provides a Material Design date picker
///    interface.
///
